...
This package is intended for neural networks that are good old super-positions of linear layers and activation layers. While the code is lacking generality in some regards, we have been very careful to develop the software to allow an arbitrary number of output features.
In short, the intended use-case is a regression task in which there are multiple inputs _and_ multiple outputs. The package is also intended for relatively simple architectures: the sort of situation where you want to regress a modest ReLU network as an alternative to Gaussian process regression.
...
```
then this package should work _extremely_ well for you.
In the opposite extreme, if your architecture isn't sequential (e.g., includes skip connections), then you'll need to implement a lot of stuff youtself as a custom sub-class of `BayesianModule`.
In the intermediate scenario that your architecture _is_ sequential but there's a convolutional layer in there, around half the classes in this package will accept your architecture but run a bit slow, meanwhile the `MixturePrior2015BNN`, `SteinEnsemble`, and `SequentialSteinEnsemble` classes will tell you the architecture isn't supported.
...
## Missing Features
 ...
 - **More Efficient Implementaiton of Non-Linear Traininable Layers:** This is probably the most egregious missing feature, at the moment. If your architecture has a `Conv2d` layer, or other trainable layer that isn't `nn.Linear`, then the code will be somewhat slow, and few options will not be supported, at all: namely the `MixturePrior2015BNN`, `SteinEnsemble`, and `SequentialSteinEnsemble` classes.
