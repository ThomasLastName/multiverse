...
class BNN(GaussianBNN):
    ...
    def resample_measurement_set(self,n=64):
        ...
        self.measurement_set = torch.rand( size=(n,1), device=device, dtype=dtype )*2-1
...
for Jac in (True,False):
    ...
    model = nn.Sequential( nn.Linear(1,1000), nn.ReLU(), nn.Linear(1000,1000), nn.ReLU(), nn.Linear(1000,1) )
    bnn = BNN(*model).to(device)
    optimizer = torch.optim.Adam( bnn.parameters(), lr=0.0001 )
    desc = "fBNN Computing the Full Jacobian" if Jac else "fBNN Without Computing the Jacobian Explicitly"
    with support_for_progress_bars():
        ...
        for iter in range(n_epochs):
            ...
            if Jac:
                with torch.no_grad():
                    _, posterior_score, prior_score = bnn.functional_kl( return_raw_ingredients=True )
                    g = posterior_score - prior_score
                    J = compute_Jacobian_of_flattened_model( bnn, bnn.measurement_set, out_features=1 )
...
print(f"The computation time of our poposed method is {1+time_trick/time_Jac:.2f} time as fast.")
