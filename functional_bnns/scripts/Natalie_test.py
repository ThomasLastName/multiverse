...
for _ in trange(epochs):
    ...
    if (_+1)%50 == 0:
        if track_pars:
            means = torch.row_stack([ means, flatten_parameters(bnn.posterior_mean).clone().detach() ])
            stds  = torch.row_stack([ stds,  flatten_parameters(bnn.posterior_std).clone().detach()  ])
        if make_gif:
            fig, ax = plot(fig,ax)
            gif.capture()
