...
class SteinEnsemble(nn.Module):
    def __init__( self, list_of_NNs, likelihood_std=None, bw=None, Optimizer=None ):
        ...
        with torch.no_grad():
            ...
            if likelihood_std is not None:
                ...
                likelihood_std = likelihood_std if isinstance(likelihood_std,torch.Tensor) else torch.tensor(likelihood_std)
    ...
    def compute_affine_transform( self, naive_implementation=False, iterative_sum=False ):
        ...
        if self.bw is None: self.bw = bandwidth_estimator(all_params,all_params)
        #
    ...
    def log_likelihood_density( self, X, y, naive_implementation=False, forward_method="bmm" ):
        ...
        if naive_implementation:
            ...
            return torch.stack([ log_gaussian_pdf( where=y, mu=model(X), sigma=self.likelihood_std ) for model in self.models ])
        #
    ...
    def log_prior_density( self, naive_implementation=False ):
        ...
        # ~~~ A more complicated, but faster implementation computes the log likelihood densities for all models "simultaneously," instead of in a loop
            ...
            marginal_log_priors = -(where/sigma)**2/2 - torch.log( math.sqrt(2*torch.pi)*sigma )
    ...
    def apply_chain_rule_for_SVGD( self, naive_implementation=False ):
        with torch.no_grad():
            ...
            for i, model in enumerate(self.models): set_flat_grads( model, stein_grads[i] )
    #
