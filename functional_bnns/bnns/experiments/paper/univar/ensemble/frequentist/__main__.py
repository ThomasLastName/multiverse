...
from bnns.experiments.paper.univar.ensemble.frequentist import folder_name
...
if len(list_of_json_files)==0: raise OSError(f"Unable to Locate any .json files in directory {parent_folder}. This is likely because `__main__.py` was not run in that folder.")
...
for hyperparameter_template in list_of_json_files:
    ...
    hyperparameter_template["BAYESIAN"] = False
    hyperparameter_template["STEIN"] = False
    hyperparameter_template["LIKELIHOOD_STD"] = 0
    hyperparameter_template["EXTRA_STD"] = False
    #
