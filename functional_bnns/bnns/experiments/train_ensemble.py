...
DTYPE = getattr(torch,hpars["DTYPE"])           # ~~~ e.g., DTYPE=="float" (str) -> DTYPE==torch.float (torch.dtype) 
...
try:    model = import_module(f'bnns.models.{hpars["ARCHITECTURE"]}')   # ~~~ this is equivalent to `import bnns.models.<hpars["ARCHITECTURE"]> as model`
except: model = import_module(hpars["ARCHITECTURE"])                    # ~~~ this is equivalent to `import <hpars["ARCHITECTURE"]> as model` (works if <hpars["ARCHITECTURE"]>.py is in the cwd or anywhere on the path)
...
if hpars["DROPOUT"] is not None:
    assert len(hpars["DROPOUT"])==hpars["N_MODELS"], f'If hpars["DROPOUT"] is a list, it must have length equal to hpars["N_MODELS"]={hpars["N_MODELS"]}'
    DROPOUT = convert_to_list_and_check_items( hpars["DROPOUT"], classes=(float,int), other_requirement=lambda p: 0<=p<=1 )
...
if isinstance(hpars["LR"],list):
    assert len(hpars["LR"])==hpars["N_MODELS"], f'If hpars["LR"] is a list, it must have length equal to hpars["N_MODELS"]={hpars["N_MODELS"]}'
...
if hpars["BAYESIAN"]:
    ...
    if hpars["STEIN"]:
        my_warn('The settings hpars["STEIN"]=True and hpars["BAYESIAN"]=False are incompatible. The former will be ignored.')
    else:
...
if data_is_univariate:
    ...
    def plot_ensemble( fig, ax, grid, green_curve, x_train_cpu, y_train_cpu, ensemble, extra_std=(hpars["LIKELIHOOD_STD"] if hpars["EXTRA_STD"] else 0.), how_many_individual_predictions=hpars["HOW_MANY_INDIVIDUAL_PREDICTIONS"], title=title ):
        #
...
if not isinstance(hpars["WEIGHTING"],str):
    my_warn(f'Expected hpars["WEIGHTING"] to be a string, but found instead type(hpars["WEIGHTING"])=={type(hpars["WEIGHTING"])}. The loss function will be weighted as if hpars["WEIGHTING"]="standard".')
...
elif hpars["WEIGHTING"]=="Sun in principle": # (EQUIVALENT TO THE "standard" WEIGHTING BELOW)
    #
    ...
    with support_for_progress_bars():   # ~~~ this just supports green progress bars
        ...
        for e in range( target_epochs - epochs_completed_so_far ):
            for b, (X,y) in enumerate(dataloader):
                ...
                if hpars["BAYESIAN"] and hpars["STEIN"]: ensemble.apply_chain_rule_for_SVGD()
                optimizer.step()
                optimizer.zero_grad()
                #
        ...
        for use_quantiles in (True,):   # ~~~ if (True,False), then the code hangs when hpars["MAKE_GIF"]==True and hpars["DEVICE"]=="cuda"!!! what the f
...
if hpars["SHOW_DIAGNOSTICS"]:
    if hpars["BAYESIAN"]: plot( "log_prior_curve", title="Prior Probabiliy as Training Progresses" )
