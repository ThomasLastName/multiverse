...
from bnns.utils.handling import load_trained_model_from_dataframe, my_warn, process_for_saving
        ...
        for i in range(n_curves):
            try:    # ~~~ try to just call curves[i] on grid
                ...
                try:    # ~~~ except, if that doesn't work, then assume we're in pytorch, 
                    assumed_device = "cuda" if torch.cuda.is_available() else "cpu"  # ~~~ assume that the curve is on "the best device available"
